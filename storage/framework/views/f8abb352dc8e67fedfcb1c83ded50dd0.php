...
function assignAbstractReviewers() {
    ...
    fetch('<?php echo e(route('assign.proposal.massReviewer')); ?>', {
        ...
        body: JSON.stringify({
            researchSubmissions: selectedAbstracts,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProposals(page = 1, search = '') {
        try {
            setLoading(true); // Show loading spinner
            const token = localStorage.getItem('auth_token');
            const response = await fetch(`/admin/api/research-submissions?page=${page}&search=${search}`, {
                method: 'GET',
                headers: {
    ...
    function renderProposals(data) {
        ...
        const rows = data.data.map(researchSubmission => {
            ...
            const avgScore = researchSubmission.average_score ? parseFloat(researchSubmission.average_score).toFixed(0) + '%': '<span class="text-gray-500">Not reviewed</span>';
            return`
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    ...
                    <div class="text-sm text-gray-500">${researchSubmission.serial_number}</div>
                </td>
                <td class="px-6 py-4 text-sm text-gray-500">${researchSubmission.user_reg_no}</td>
                <td class="px-6 py-4 text-sm text-gray-500">
                    ${new Date(researchSubmission.created_at).toLocaleDateString('en-US', {
                        year: 'numeric', // 2025
        ...
        paginationDiv.innerHTML = `
            <button ${data.prev_page_url ? '' : 'disabled'} class="px-4 py-2 bg-gray-200 rounded-lg" onclick="fetchProposals(${data.current_page - 1})">Previous</button>
            <button ${data.next_page_url ? '' : 'disabled'} class="px-4 py-2 bg-gray-200 rounded-lg" onclick="fetchProposals(${data.current_page + 1})">Next</button>
        `;
    ...
    searchButton.addEventListener('click', () => {
        fetchProposals(1, searchInput.value);
    });
});
</script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\MSS\mss-project\resources\views/admin/partials/proposal.blade.php ENDPATH**/ ?>