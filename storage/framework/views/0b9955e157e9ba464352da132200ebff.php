            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <span class="px-3 py-1 text-xs font-semibold text-<?php echo e($type->status === 'active' ? 'green' : 'red'); ?>-700 bg-<?php echo e($type->status === 'active' ? 'green' : 'red'); ?>-100 rounded-full">
                        <?php echo e($type->status === 'active' ? 'Open' : 'Closed'); ?>

                    </span>
                    <span class="text-sm text-gray-500">Deadline: <?php echo e(\Carbon\Carbon::parse($type->deadline)->format('M d, Y')); ?></span>
                </div>
                
                <h2 class="text-xl font-bold text-gray-800 mb-3"><?php echo e($type->title); ?></h2>
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        Format: <?php echo e($type->format); ?>

                    </div>
                </div>

                <a href="<?php echo e($type->status === 'active' ? ($type->type === 'conference' ? route('user.step1') : route('user.step1_research')) : '#'); ?>"
                    class="block w-full py-3 px-4 text-center font-medium rounded-lg transition-colors duration-200 
                    <?php echo e($type->status === 'active' 
                        ? ($type->type === 'conference' ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-green-600 hover:bg-green-700 text-white')
                        : 'bg-gray-300 text-gray-500 cursor-not-allowed'); ?>"
                    <?php echo e($type->status !== 'active' ? 'onclick="return false;" aria-disabled="true"' : ''); ?>>
                    Submit <?php echo e($type->type === 'conference' ? 'Conference' : 'Research'); ?> Paper
                </a>
            </div>

            <?php if($type->guidelines): ?>
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <h3 class="text-sm font-semibold text-gray-800 mb-2">Important Guidelines:</h3>
                <ul class="list-disc list-inside text-sm text-gray-600 space-y-1">
                    <?php $__currentLoopData = explode("\n", $type->guidelines); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $guideline): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li><?php echo e($guideline); ?></li>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\MSS\mss-project\resources\views/user/partials/submit.blade.php ENDPATH**/ ?>